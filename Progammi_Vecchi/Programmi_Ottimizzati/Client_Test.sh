...
for dim in "${dim_bw[@]}" #loop che varia il traffico generato dal client iperf 
  do
    ...
    start_iperf_client $IPERF_SERVER_IP $IPERF_CLIENT_PORT $IPERF_CLIENT_DURATION $dim 
    #salvataggio del pid del processo client iperf 
    #in modo da poter sapere quando e' terminato
    pid=$!
    for param1 in "${dim_payload[@]}" #loop che varia la dimensione dei payload 
      ...
      do
        ...
        .. --payload_size "$param1" --type_test "$dim" &
